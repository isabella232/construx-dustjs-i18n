...
module.exports = function (options) {
    ...
    return function (data, args, callback) {
        var srcPath, propPath;

        try {
            //if there is i18n, need to first run through localizr
            if (options.i18n) {
                srcPath = args.context.srcPath;
                propPath = path.join(options.i18n.contentPath, args.context.name + '.properties');
                i18n.localize(srcPath, propPath, function(err, data) {
                    if (err) {
                        callback(err);
                        return;
                    }
